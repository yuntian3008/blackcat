          ...
          <div class="text-gray-600">
            <p class="font-medium text-lg my-3">Avatar</p>
          	<label for="avatar" class="w-36 block mx-auto cursor-pointer relative hover:opacity-50">
          		...
          		@if ( Auth::user()->avatar)
                  <img class="mx-auto my-3 object-cover rounded-full h-36 w-36 " src="{{ Auth::user()->getAvatar() }}" alt="avatar">
              @elseif(Auth::user()->getFullName())
                  <img class="mx-auto my-3 object-cover rounded-full h-36 w-36 " src="{{ \Avatar::create(Auth::user()->getFullName())->toBase64() }}" alt="avatar">
              @else
                  <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto my-3 object-cover rounded-full h-36 w-36 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
